...
class TelaReserva(TelaAbstrata):
  ...
  def pega_dados_reserva(self, modo: str = "cadastro", dados_atuais: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
    ...
    layout = [
      ...
      [sg.Text('CPFs do(s) Hóspede(s) (separados por vírgula):', size=(35, 1)), sg.InputText(hospedes_val, key='hospedes_cpfs')],
      [sg.Text('Numero do(s) Quarto(s) (separados por vírgula):', size=(35, 1)), sg.InputText(quartos_val, key='quartos_ids')],
      [sg.Text('Data Check-in (DD/MM/AAAA):', size=(35, 1)), sg.InputText(checkin_val, key='checkin_data')],
      [sg.Text('Data Check-out (DD/MM/AAAA):', size=(35, 1)), sg.InputText(checkout_val, key='checkout_data')],
      [sg.Button('Confirmar'), sg.Button('Cancelar')]
    ]
  ...
  def pega_dados_pet(self) -> Optional[Dict[str, Any]]:
          ...
          layout = [
            ...
            [sg.Text('Nome do Pet:', size=(30, 1)), sg.InputText('', key='nome_pet')],
            [sg.Text('Espécie do Pet (Ex: Cachorro, Gato):', size=(30, 1)), sg.InputText('', key='especie')], 
            [sg.Button('Confirmar'), sg.Button('Cancelar')]
          ]
